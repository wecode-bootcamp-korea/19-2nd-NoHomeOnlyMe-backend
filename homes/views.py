        ...
        try:
            ...
            result = {
                ...
                "detail_header" : {
                    ...
                    "month_total_cost" : (int(room.saleinformation_set.first().monthly_pay) + int(room.additionalinformation_set.first().maintenance_cost) + int(room.additionalinformation_set.first().parking_fee))/10000,
            ...
            if Home.objects.filter(user_id = room.user_id).count() > 1:
                ...
                } for other_room in Home.objects.filter(user_id = room.user_id) if other_room.id != home_id]
